...
Each faction has a palette that can be spawned in using the `mi` command followed by either its [`group`](./blocks.lua.html?highlight=group) in the `blocks.lua` or its [`name`](./factions.lua.html?highlight=name) in the `factions.lua`.
...
Spawn in a faction's fleets by using the `fleetp` command followed by either its [faction ID](./factions.lua.html?highlight=98) or its [`name`](./factions.lua.html?highlight=name) in the `factions.lua`.
